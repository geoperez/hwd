...
namespace HWD
{
	...
	public class Details : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.tabPage1.Controls.Add(this.treeSysInfo);
			...
			this.treeSysInfo.FullRowSelect = true;
			this.treeSysInfo.ImageList = this.imageList1;
			this.treeSysInfo.Location = new System.Drawing.Point(296, 8);
			this.treeSysInfo.Name = "treeSysInfo";
			this.treeSysInfo.Size = new System.Drawing.Size(304, 280);
			this.treeSysInfo.TabIndex = 11;
			// 
			// groupBox1
			// 
			...
			this.button6.Location = new System.Drawing.Point(490, 8);
			...
			this.button9.Location = new System.Drawing.Point(490, 8);
			...
			this.button7.Location = new System.Drawing.Point(490, 8);
			...
			// lstPro
			// 
			this.lstPro.BackColor = System.Drawing.Color.OldLace;
			...
			this.button12.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			this.button12.ForeColor = System.Drawing.Color.OldLace;
			this.button12.Location = new System.Drawing.Point(490, 8);
			...
			this.button8.Location = new System.Drawing.Point(490, 8);
			...
			this.button11.Location = new System.Drawing.Point(490, 8);
		...
		private void scanMore()
		{
			...
			try 
			{
				nodeCollection = createParentNode("Processors", 0);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_Processor"))
				{
					createChildNode(nodeCollection, "Caption: " + mo["Caption"], 0);
					this.rpProcessors = mo["Caption"] + "\n\t\t" + this.rpProcessors;
				}
				this.Update();

				nodeCollection = createParentNode("Network Adapters", 1);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_NetworkAdapter"))
				{
					createChildNode(nodeCollection, "NIC Name: " + mo["ProductName"], 1);
					createChildNode(nodeCollection, "MAC Address: " + mo["MACAddress"], 1);
					this.rpNICs = mo["ProductName"] + " (" + mo["MACAddress"] + ")\n\t\t" + this.rpNICs;
				}
				this.Update();

				nodeCollection = createParentNode("Video Adapters & Monitors", 2);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_DesktopMonitor"))
				{
					createChildNode(nodeCollection, "Monitor: " + mo["MonitorType"], 2);
				...
				{
					createChildNode(nodeCollection, "Video Card: " + mo["Caption"], 2);
					createChildNode(nodeCollection, "BPP: " + mo["CurrentBitsPerPixel"], 2);
				}
				this.Update();

				nodeCollection = createParentNode("Printers", 3);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_Printer"))
				{
					createChildNode(nodeCollection, "Name: " + mo["Caption"], 3);
					createChildNode(nodeCollection, "Location: " + mo["Location"], 3);
					this.rpPrinters = mo["Caption"] + "\n\t\t" + this.rpPrinters;
				}
				this.Update();

				nodeCollection = createParentNode("Disk Drives", 4);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_DiskDrive"))
				{
					createChildNode(nodeCollection, "Caption: " + mo["Caption"], 4);
					createChildNode(nodeCollection, "Size: " + formatSize(Convert.ToInt64(mo["Size"].ToString()),false), 4);
					this.rpHDs = mo["Caption"] + " (" + formatSize(Convert.ToInt64(mo["Size"].ToString()),false) + ")\n\t\t" + this.rpHDs;
				}
				this.Update();

				nodeCollection = createParentNode("CD-ROM Drives", 5);
				foreach (ManagementObject mo in Consulta("SELECT * FROM Win32_CDROMDrive"))
				{
					createChildNode(nodeCollection, "Caption: " + mo["Caption"], 5);
					createChildNode(nodeCollection, "Drive: " + mo["Drive"], 5);
		...
		private TreeNodeCollection createParentNode(string stringName, int id)
		{
			TreeNode nodeTreeNode = new TreeNode(stringName, id, id);
			treeSysInfo.Nodes.Add(nodeTreeNode);
			return nodeTreeNode.Nodes;

		}
		private void createChildNode(TreeNodeCollection nodeCollection, string stringCaption, int id)
		{
			TreeNode nodeTreeNode = new TreeNode(stringCaption, id, id);
